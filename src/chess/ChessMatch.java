    ...
    private Piece makeMove(Position source, Position target) {
        ...
        if (p instanceof Pawn) {
            if (source.getCollumn() != target.getCollumn() && capturedPiece == null) {
                Position pawnPositin;
                if (p.getColor() == Color.WHITE) {
                    pawnPositin = new Position(target.getRow() + 1, target.getCollumn());
                } else {
                    pawnPositin = new Position(target.getRow() - 1, target.getCollumn());
                }
                capturedPiece = board.removePiece(pawnPositin);
                capturedPieces.add(capturedPiece);
                piecesOnTheBoard.remove(capturedPiece);
            }
        }
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        if (p instanceof Pawn) {
            if (source.getCollumn() != target.getCollumn() && capturedPiece == enPassantVulnerable) {
                ...
                Position pawnPositin;
                if (p.getColor() == Color.WHITE) {
                    pawnPositin = new Position(3, target.getCollumn());
                } else {
                    pawnPositin = new Position(4, target.getCollumn());
                }
                board.placePiece(pawn, pawnPositin);
            }
        }
